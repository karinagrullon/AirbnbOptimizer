...
$sql = "SELECT AVG(round(price)) as newPrice,neighbourhood_cleansed,latitude,longitude, room_type,id,room_type FROM listings WHERE price >= 0 and price <= 1000 && room_type='Entire home/apt' GROUP BY neighbourhood_cleansed,room_type";
/*
...
while ($row = $rs->fetch(PDO::FETCH_ASSOC)) {
	...
	if ($row['newPrice'] >= 125 && $row['newPrice'] <= 167) {
		$weight = 2;
	} elseif ($row['newPrice'] >= 168 && $row['newPrice'] <= 209) {
						$weight = 3;
	} elseif ($row['newPrice'] >= 210 && $row['newPrice'] <= 251) {
							$weight = 4;
	} elseif ($row['newPrice'] >= 252 && $row['newPrice'] <= 293) {
							$weight = 5;
	} elseif ($row['newPrice'] >= 294 && $row['newPrice'] <= 335) {
							$weight = 6;
	}
