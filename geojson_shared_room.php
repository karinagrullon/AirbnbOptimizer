...
$sql = "SELECT AVG(round(price)) as newPrice,neighbourhood_cleansed,latitude,longitude, room_type,id,room_type FROM listings WHERE price >= 0 and price <= 1000 && room_type='Shared room' GROUP BY neighbourhood_cleansed,room_type";
/*
...
// Getting weight value for heatmap according to listing price
	if ($row['newPrice'] >= 28 && $row['newPrice'] <= 59) {
		$weight = 2;
	} elseif ($row['newPrice'] >= 60 && $row['newPrice'] <= 90) {
						$weight = 3;
	} elseif ($row['newPrice'] >= 91 && $row['newPrice'] <= 121) {
							$weight = 4;
	} elseif ($row['newPrice'] >= 122 && $row['newPrice'] <= 152) {
							$weight = 5;
	} elseif ($row['newPrice'] >= 153 && $row['newPrice'] <= 187) {
							$weight = 6;
	}
